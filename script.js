document.addEventListener('DOMContentLoaded', () => {
    ...
    // NOVO: Referência ao loader de tela cheia
    ...
    connectIpButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (saude.status === "ok") {
                ...
                setTimeout(() => {
                    ipConfigModal.style.display = 'none';
                    // Chama a função de busca após a conexão, ativando o loader
    ...
    async function fetchMusicData(ip) {
        ...
        // NOVO: Ativa o loader antes de iniciar a busca
        ...
        try {
            ...
            // NOVO: Desativa o loader após a busca (sucesso ou erro)
...
// Função para alternar a classe 'open' em telas pequenas (mobile/tablet)
// E a classe 'collapsed' em telas grandes (desktop)
function toggleSidebar() {
    // 900px é o ponto de quebra que definimos no CSS
    if (window.innerWidth <= 900) {
        // Modo Mobile/Tablet: Usa a classe 'open' para deslizar o menu
        sidebar.classList.toggle('open');
        // Você pode também querer mudar o ícone do menu aqui
        menuToggle.querySelector('.material-icons').textContent = 
            sidebar.classList.contains('open') ? 'close' : 'menu';
    } else {
        // Modo Desktop: Usa a classe 'collapsed' para encolher
        sidebar.classList.toggle('collapsed');
        
        // Se a sidebar colapsou, a logo e texto somem.
        // Se expandiu, eles aparecem.
        // Você pode adaptar a lógica aqui se precisar de algo mais refinado.
    }
}
...
// Opcional: Garante que o estado inicial da sidebar esteja correto ao carregar
// E ajusta se a tela for redimensionada de mobile para desktop
window.addEventListener('resize', () => {
    if (window.innerWidth > 900) {
        // Garante que o menu está no modo padrão expandido (se não estiver colapsado) 
        // ou colapsado, e remove a classe 'open' (modo mobile)
        sidebar.classList.remove('open');
        
        // Garante que o ícone volte a ser "menu" se estava "close"
        menuToggle.querySelector('.material-icons').textContent = 'menu';
    } else {
         // Garante que o menu está no modo padrão "colapsado" em mobile (totalmente fora)
         sidebar.classList.remove('collapsed');
    }
});

// Força o estado inicial para desktop
if (window.innerWidth > 900) {
    // Começa com o menu colapsado se desejar, mas vamos manter expandido por padrão
    // se nenhuma classe foi definida.
}